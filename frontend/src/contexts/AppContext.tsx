...
export function useCart() {
  ...
  const addToCart = async (book: Book) => {
    ...
    // Schedule cart abandonment reminder (handled by notification system)
    try {
      const event = new CustomEvent("cartUpdated", {
        detail: { action: "add", cart: [...state.cart, { book, quantity: 1 }] },
      });
...
export function useWishlist() {
  ...
  const addToWishlist = async (book: Book) => {
    ...
    if (state.user) {
      try {
        await apiClient.addToWishlist(book.id);
      } catch (error) {
  ...
  const syncWishlist = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        // Transform backend wishlist data to local format
        const syncedWishlist = response.data.map((item: any) => ({
          book: {
            id: item.bookId,
            title: item.title,
            price: item.price,
        ...
        dispatch({ type: "SYNC_WISHLIST", payload: syncedWishlist });
      }
    } catch (error) {
