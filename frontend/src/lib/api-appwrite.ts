...
const isOfflineMode =
  isDevelopment && endpoint === "https://cloud.appwrite.io/v1";

// Track if we've already shown the development notice
let developmentNoticeShown = false;

function showDevelopmentNotice() {
  if (!developmentNoticeShown && import.meta.env.VITE_DEBUG === "true") {
    console.log(
      "🚀 Development Mode: Using mock data due to Appwrite connection issues",
    );
    developmentNoticeShown = true;
  }
}
...
// Books API (using Appwrite Functions with Database fallback)
export const booksAPI = {
  async getBooks(
    ...
    // Skip functions completely in offline development mode
    if (isOfflineMode) {
      console.log(
        "🔄 Development mode: Bypassing functions, using database fallback for getBooks",
      );
      showDevelopmentNotice();
      return await DatabaseHelpers.getBooks(filters);
    }

    try {
      // Try functions first, fallback to database
      try {
        ...
        console.warn(
          "Functions not available, using database fallback:",
          funcError,
        );
        showDevelopmentNotice();
  ...
  async getBook(id: string) {
    ...
    if (isOfflineMode) {
      console.log(
        "🔄 Development mode: Bypassing functions, using database fallback for getBook",
      );
      return await DatabaseHelpers.getBook(id);
    }
  ...
  async createBook(bookData: any) {
    ...
    if (isOfflineMode) {
      console.log(
        "🔄 Development mode: Bypassing functions, using database fallback for createBook",
      );
      return await DatabaseHelpers.createBook(bookData);
    }
  ...
  async updateBook(id: string, bookData: any) {
    try {
      try {
        ...
        return await DatabaseHelpers.updateBook(id, bookData);
      }
    } catch (error) {
      throw new AppwriteError(
  ...
  async deleteBook(id: string) {
    try {
      try {
        ...
        return await DatabaseHelpers.deleteBook(id);
      }
    } catch (error) {
      throw new AppwriteError(
...
export const ordersAPI = {
  ...
  async getAllOrders(
    ...
    if (isOfflineMode) {
      console.log(
        "🔄 Development mode: Bypassing functions, using database fallback for getAllOrders",
      );
      showDevelopmentNotice();
      return await DatabaseHelpers.getAllOrders(filters);
    }
