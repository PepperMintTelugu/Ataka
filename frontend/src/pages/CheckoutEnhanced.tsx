...
import { paymentAPI } from "@/lib/api-appwrite";
...
export default function CheckoutEnhanced() {
  ...
  const processRazorpayPayment = async () => {
    try {
      ...
      // Create order using Appwrite API
      const orderResponse = await paymentAPI.createRazorpayOrder({
        ...
        userId: "guest", // TODO: Get actual user ID when auth is implemented
      });
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            // Verify payment using Appwrite API
            ...
            if (verifyData && verifyData.order) {
              ...
              navigate(
                `/order-success/${verifyData.order.id || orderData.order.id}`,
              );
            } else {
