...
import { ordersAPI, settingsAPI } from "@/lib/api-appwrite";
...
export default function ShippingManager() {
  ...
  const loadShipments = async () => {
    try {
      setLoading(true);
      const orders = await ordersAPI.getAllOrders(); // Get orders with shipping info
      // Transform orders to shipments format
      const shipmentsData = orders
  ...
  const syncWithShiprocket = async () => {
    try {
      ...
      toast({
        ...
        description: "Shipment statuses refreshed from backend.",
      });
    } catch (error) {
      console.error("Sync failed:", error);
      toast({
        title: "Sync Failed",
        description: "Failed to refresh shipment data. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsSyncing(false);
  ...
  const saveSettings = async () => {
    try {
      await settingsAPI.updateSetting("shipping", JSON.stringify(settings), "json");
      toast({
