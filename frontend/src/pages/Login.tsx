...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // The OAuth flow will redirect to /auth/success
    } catch (error: any) {
      console.error("Google login error:", error);

      toast({
        title: "Sign In Failed",
        description: error.message || "Failed to start Google sign in process",
        variant: "destructive",
      });
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If not admin, try customer login with Appwrite
